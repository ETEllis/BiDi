...
`bridge512.cdc`, `bridge4096.cdc`, `bridge_jobs.cdc`, `native_reducer.cdc`,
`native_surface.cdc`, `council_bridge.cdc`, `system.cdc`, `relations.cdc`, and
`trace_windows.cdc`.
The bridge codebooks are also consumed and regenerated by the non-Python runtime
...
64-row `2^6 = 4^3` dyadic/triadic bridge codebook; `bridge512.cdc` and
`bridge4096.cdc` are the full generated `n=9` and `n=12` higher-arity rows; and
`runtime/cdc_bridge_runtime.c` consumes them for lookup, trace-coordinate
projection, generated-codebook verification, and interactive grid generation.
The end
...
Lean/Coq source together with finite algebraic law checks. These checks do not
prove industrial performance, scaling limits, continuous numeric accuracy, or
optimality.
