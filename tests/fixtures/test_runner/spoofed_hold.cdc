# spoofed_hold.cdc -- review B3 permanent negative fixture: identical to
# silent_hold.cdc plus an UNRELATED witness statement carrying the held
# job's name and expect-status=held. A witness is not the executable
# statement that produced the hold; it must never authorize it. The typed
# gate must still fail with an unexpected hold.
...
witness silent-hold expect-status=held claim="unrelated witness must not authorize runtime hold"
