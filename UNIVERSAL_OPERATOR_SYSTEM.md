...
```mermaid
flowchart TB
    Contract["CDC .cdc contract"] --> Parse["typed AST + source digest"]
    Parse --> State["native continuous + discrete state"]
    State --> Primitive["flow · commit · nest"]
    Primitive --> U1["U1 acceptance + effect receipt"]
    U1 --> Tangent["U2 path tangent D U"]
    Tangent --> Recurrence{"full or explicit relative recurrence"}
